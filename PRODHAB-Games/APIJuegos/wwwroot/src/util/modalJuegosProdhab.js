const ModalMenuJuegosPRODHAB = (() => {
  ...
  function aplicarEstilosResponsive(closeBtn) {
    function ajustar() {
  ...
  async function openAutoModal(htmlContent) {
  ...
  // Abrir el modal usando la configuración actual
  function openAutoModalGames(idJuego, idTipoJuego) {
    const tipo = Number(idTipoJuego);
    const config = juegosAssets.getAtributos(tipo);
    if (!config) return;
  ...
  return { openAutoModal, openAutoModalGames};
})();
