...
@media (max-width: 480px) {
  ...
  mostrarJuegos() {
    ...
    juegosPagina.forEach(juego => {
      ...
      card.className = "card";
      card.innerHTML = `
            <div class="card-header"><h3>${utilHtmlJuegos.escapeHtml(juego.nombre)}</h3></div>
            <div class="card-body">
              <p><strong>Descripción:</strong> ${utilHtmlJuegos.escapeHtml(juego.descripcion) || "Sin descripción"}</p>
              <p><strong>Instrucciones:</strong> ${utilHtmlJuegos.escapeHtml(juego.detalle) || "N/A"}</p>
