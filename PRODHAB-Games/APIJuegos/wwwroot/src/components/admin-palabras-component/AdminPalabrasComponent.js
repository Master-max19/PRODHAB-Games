...
`;
  ...
  connectedCallback() {
    this.dispatchEvent(new CustomEvent(this.id || "admin-palabra-component", { bubbles: true }));
  ...
  _bindItemEvents(el, item, absIdx) {
    ...
    if (addSubBtn && subInput) {
      addSubBtn.addEventListener("click", () => {
        ...
        if (!val) {
          utilModalJuegos.mostrarMensajeModal("Aviso", this._texts.emptySubItem);
  ...
  _render() {
    ...
    if (total === 0) {
      ...
      slice.forEach((item, relIdx) => {
        ...
        if (state.editing) {
          el.innerHTML = `
            <textarea class="edit-title">${utilHtmlJuegos.escapeHtml(item.titulo)}</textarea>
            <div class="save-row">
          ...
          el.querySelector(".save-sub-btn").addEventListener(
            ...
            () => {
              ...
              if (!val) {
                utilModalJuegos.mostrarMensajeModal("Aviso", this._texts.emptyTitle);
          ...
          el.innerHTML = `
            <div class="title-row">
              <div class="title-text"><span class="check">✔</span><div class="title-content">${utilHtmlJuegos.escapeHtml(item.titulo)
            }</div></div>
            </div>
        <div class="btn-row">
  ...
  >
    ${this._texts.deleteButton}
            ...
            <div class="sublist">
              ...
              .map(
                (s) =>
                  `<div class="chip" data-id="${s.id}"><span>${utilHtmlJuegos.escapeHtml(s.texto)}</span><button>×</button></div>`
              )
              .join("")}
              ${state.pendingSubItems
              .map(
                (s) =>
                  `<div class="chip pending-chip" data-id="${s.id}"><span>${utilHtmlJuegos.escapeHtml(s.texto)}</span><button>×</button></div>`
              )
              .join("")}
            </div>
            <div class="add-sub">
