...
const resultadoJuegoService = (() => {
     ...
     */
    async function registrarJuego(idJuego) {
        try {
            const response = await fetch(`${CONFIG_JUEGO_PRODHAB.apiUrl}/registrar/${idJuego}`, {
                method: 'POST',
     ...
     * @returns {Promise<Object|null>} - Datos de la estadística o null si hay error
     */
    async function obtenerEstadisticas(idJuego = 17) {
        try {
            const data = await utilFetch.apiFetch(`${CONFIG_JUEGO_PRODHAB.apiUrl}/estadisticas/${idJuego}`, {
                method: 'GET',
                headers: { 'Accept': '*/*' },
            });
    ...
    return { registrarJuego, obtenerEstadisticas }
