 ...
 */
 ...
 obtenerCantidadPreguntasCorrecta(detalle) {
    if (!Array.isArray(detalle)) return 0;

    try {
        return detalle.filter(pregunta => {
            let todasCorrectas = true;
            let ningunaIncorrecta = true;

            for (const op of pregunta.opciones) {
                if (op.esCorrecta && !op.seleccionada) todasCorrectas = false;
                if (!op.esCorrecta && op.seleccionada) ningunaIncorrecta = false;

                if (!todasCorrectas || !ningunaIncorrecta) break; // salir temprano
            }

            return todasCorrectas && ningunaIncorrecta;
        }).length;
    } catch (error) {
        console.error("Error al contar preguntas correctas:", error);
        return 0;
    }
}
