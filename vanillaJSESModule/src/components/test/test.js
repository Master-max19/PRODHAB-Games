...
export class Test {
  ...
  responder(idPregunta, idOpcion) {
    ...
    const pregunta = this.preguntas.find(
      (p) => Number(String(p.id).replace(/^pregunta/, "")) === preguntaId
    );
    if (!pregunta) {
    ...
    const opcion = pregunta.opciones.find(
      (o) => Number(String(o.id).replace(/^o/, "")) === opcionId
    );
    if (!opcion) {
      console.warn(
        `Opción no encontrada: idOpcion=${opcionId}, idPregunta=${preguntaId}`
      );
      return;
    }

    if (pregunta.categoria === "multiple") {
      let respuesta = this.respuestasUsuario.find(
        (r) => r.idPregunta === preguntaId
      );
      if (!respuesta) {
      ...
      const index = this.respuestasUsuario.findIndex(
        (r) => r.idPregunta === preguntaId
      );
  ...
  obtenerRespuestas() {
    const respuestas = this.preguntas
      .map((pregunta) => {
        const preguntaId = Number(String(pregunta.id).replace(/^pregunta/, ""));
        const respuesta = this.respuestasUsuario.find(
          (r) => r.idPregunta === preguntaId
        );

        const opciones = pregunta.opciones.map((opcion) => ({
          idOpcion: Number(String(opcion.id).replace(/^o/, "")),
          seleccionada: respuesta
            ? pregunta.categoria === "multiple"
              ? respuesta.idOpciones?.includes(
                Number(String(opcion.id).replace(/^o/, ""))
              ) || false
              : respuesta.idOpcion ===
              Number(String(opcion.id).replace(/^o/, ""))
            : false,
