class OrdenaLetrasComponent extends HTMLElement {
  ...
  async connectedCallback() {
    ...
    let palabras = this.getAttribute("words") ? JSON.parse(this.getAttribute("words")) : ["ejemplo"];

    let textoIntroduccion = this.getAttribute("intro-text") || "Este es un ejemplo de historia con palabras importantes.";

    let tema = this.getAttribute("tema") || "Juego no disponible";
...
.contenedor-consentimiento input[type="checkbox"] {
  accent-color: #1e355e; /* Cambia este color al que quieras */
  width: 18px;
  height: 18px;
  cursor: pointer;
}

/* Opcional: hover para checkbox */
.contenedor-consentimiento input[type="checkbox"]:hover {
  accent-color: #160752;
}
...
.contenedor-consentimiento label {
  ...
  color: #333; /* Color del texto de la etiqueta */
      ...
      <div class="contenedor-juego" tabindex="0">
        ...
        <div id="introduccion">
          <h2 id="tema">${utilHtmlJuegos.escapeAttr(this.getAttribute("tema")) || "Ordena las palabras"}</h2>
          <p id="historia"></p>
  ...
  resaltarPalabras(texto, palabras) {
    ...
    let resultado = utilHtmlJuegos.escapeHtml(texto);
  ...
  startConfetti() {
    ...
    const particles = [];
    const colors = ["#ADD8E6", "#4682B4", "#0000FF", "#1E90FF", "#87CEEB"];
  ...
  configurarEventos() {
    ...
    this.botonReiniciar.addEventListener("click", () => {
      ...
      try {
        palabrasOriginales = JSON.parse(this.getAttribute("words") || "[]").filter(
          (p) =>
            typeof p === "string" &&
            p.trim() !== "" &&
      ...
      if (this.historia) {
        ...
        if (this.temaEl) this.temaEl.textContent = this.getAttribute("tema") || "";
      }
  ...
  mostrarPalabra() {
    ...
    palabraMezclada.split("").forEach((letra) => {
      ...
      boton.addEventListener("click", () => {
        ...
        boton.disabled = true;
        this._stackBotonesElegidos.push(boton);
  ...
  _usarLetraDesdeTeclado(keyChar) {
    ...
    const botones = Array.from(this.contenedorLetras.querySelectorAll(".boton-letra"));
    const match = botones.find(
      (b) => candidates.includes(b.textContent) && !this._stackBotonesElegidos.includes(b)
    );
  ...
  _limpiarSeleccionActual() {
    this.palabraFormada.textContent = "";
    this._stackBotonesElegidos.forEach(b => {
      b.disabled = false;
