...
<style>
  :host { display:block; max-width:600px; margin:30px auto; background:white; padding:20px; border-radius:12px; box-shadow:0 4px 10px rgba(0,0,0,0.1); font-family:Segoe UI, sans-serif;}
  *, *::before, *::after { box-sizing:border-box; }
  ...
  button { background-color:#1976d2; color:#fff; border:none; padding:10px 16px; border-radius:6px; cursor:pointer; font-weight:500; transition:0.3s; }
...
<form id="eliminarUsuarioForm">
  ...
  async crearUsuario(e) {
    ...
    const rolId = parseInt(this.shadowRoot.getElementById("rolId").value);
    this.dispatchEvent(
      new CustomEvent("crear-usuario-request", {
        detail: { correo, password, rolId },
      })
    );
  }
