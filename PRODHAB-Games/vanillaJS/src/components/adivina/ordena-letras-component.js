class JuegoMemoria extends HTMLElement {
  ...
  // Definir atributos con prioridad:
  // 1️⃣ HTML
  // 2️⃣ Servicio
  ...
  let textoIntroduccion = this.getAttribute("intro-text") 
  ...
  const videoSrc = this.getAttribute("video-final-src") 
    || "video.webm";

  // Guardar en atributos si no existen
  if (!this.hasAttribute("words")) this.setAttribute("words", JSON.stringify(palabras));
  if (!this.hasAttribute("intro-text")) this.setAttribute("intro-text", textoIntroduccion);
  if (!this.hasAttribute("tema")) this.setAttribute("tema", tema);
  if (!this.hasAttribute("video-final-src")) this.setAttribute("video-final-src", videoSrc);
...
resaltarPalabras(texto, palabras) {
  ...
  palabras.forEach(palabra => {
    ...
    for (let i = indices.length - 1; i >= 0; i--) {
      ...
      resultado = resultado.slice(0, start) + "<strong>" + resultado.slice(start, end) + "</strong>" + resultado.slice(end);
    }
  });
...
this.botonReiniciar.addEventListener("click", async () => {
  ...
  try {
    ...
    textoYPalabras = { palabras: ["ejemplo"], texto: "Este es un ejemplo de historia con palabras importantes.", tema: "Protección de datos" };
  }

  // Asignar valores por defecto si los datos no están disponibles
  const palabrasOriginales = textoYPalabras.palabras && Array.isArray(textoYPalabras.palabras) 
    ? textoYPalabras.palabras 
    : ["ejemplo"];
  const textoIntroduccion = textoYPalabras.texto || "Este es un ejemplo de historia con palabras importantes.";
  ...
  try {
    palabrasFiltradas = JSON.parse(this.getAttribute("words") || "[]").filter(
      (p) => typeof p === "string" && p.trim() !== "" && /^[a-zA-ZáéíóúÁÉÍÓÚñÑüÜ]+$/.test(p)
    );
  } catch (error) {
  ...
  this.palabras = palabrasFiltradas.length > 0 
