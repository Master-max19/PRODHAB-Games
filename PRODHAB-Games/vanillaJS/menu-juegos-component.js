class MenuJuegosComponent extends HTMLElement {
  constructor() {
    super();
    this.paginaActual = 1;
    ...
    this.tiempoMaximo = 30000; // 30 segundos
...
@media (max-width: 480px) {
    ...
    margin: 1rem;
          ...
          <div class="container">
            ...
            <div class="filtros" id="filtrosContainer">
              ...
              <button class="filtro-btn" data-filtro="2">Ordenar Palabra</button>
              <button class="filtro-btn" data-filtro="3">Completar Texto</button>
  ...
  mostrarJuegos() {
    ...
    juegosPagina.forEach(juego => {
      ...
      card.className = "card";
      card.innerHTML = `
            <div class="card-header"><h3>${juego.nombre}</h3></div>
            <div class="card-body">
  ...
  // Métodos para cargar cada tipo de juego
  completar(id) {
    const cont = this.shadowRoot.getElementById("juegoContent");
    if (!cont) return;
    cont.innerHTML = `<div style="width:100%;"><completar-texto-component img-intro="public/images/superdato_1.png" webm-final="public/video/superdato_escribe.webm" img-rondas="public/images/superdato_manos_cintura.png" id-completar="${id}"></completar-texto-component></div>`;
  }

  sopa(id) {
    const cont = this.shadowRoot.getElementById("juegoContent");
    if (!cont) return;
    cont.innerHTML = `<div style="width:100%;"><sopa-letras-component size="14" id-sopa="${id}" modal1-video="public/video/superdato_anotando.webm" modal2-video="public/video/superdato_idea.webm" superdato-img="public/images/superdato_señalando.png"></sopa-letras-component></div>`;
  }

  test(id) {
    const cont = this.shadowRoot.getElementById("juegoContent");
    if (!cont) return;
    cont.innerHTML = `<test-component style-url="src/components/test/test.css" correcta_svg="public/images/correcta.svg" incorrecta_svg="public/images/incorrecta.svg" character_png="public/images/superdato_2.png" id-test="${id}"></test-component>`;
  }

  ordenar(id) {
    const cont = this.shadowRoot.getElementById("juegoContent");
    if (!cont) return;
    cont.innerHTML = `<juego-ordena-letras-component id-ordenar="${id}" video-final-src="public/video/super_dato_sonrie.webm" inicio-video-src="public/video/super_dato_saluda.webm" style="width:100%;"></juego-ordena-letras-component>`;
  }
}
